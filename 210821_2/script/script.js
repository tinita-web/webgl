...
// = 026 ======================================================================
// 画像を読み込みテクスチャとし、それをシェーダ側で参照することで表情豊かな頂点
// 表現ができるポイントスプライトですが、これをさらに応用することでより直接的に、
// シェーダからグラフィックスを出力する方法もあります。
// ここでは、参考程度に見てもらいたいのですが、すべての「模様」をシェーダ側だけ
// で動的に生成しています。
// フラグメントシェーダで光るような模様を作り、それを頂点に貼り付けます。このよ
// うなシェーダのみで模様を作るテクニックを習得すると、より柔軟な表現を行えるよ
// うになります。これは、シェーダで動的に模様を生成する場合、静止画をテクスチャ
// として用いる場合とは異なり、リアルタイムにパラメータを変更しながら模様を生成
// できるためです。
// ============================================================================
...
(() => {
    ...
    let globalColor = [0.9, 0.2, 0.5]; // 頂点に適用するマテリアルの色

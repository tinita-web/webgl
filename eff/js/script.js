(() => {
    window.addEventListener('DOMContentLoaded', () => {
        ...
        async function loadSomeTexture(){
            // await で非同期関数の完了を同期的に待つ
            earthTexture = await (() => {
                return new Promise((resolve) => {
                    loader.load('./img/earth.jpg', resolve);
                });
            })();
            // await で非同期関数の完了を同期的に待つ
            moonTexture = await (() => {
                return new Promise((resolve) => {
                    loader.load('./img/moon.jpg', resolve);
                });
            })();
            // await で同期的に解決を待つので、ここに来たということは
            // 上記の２つの Promise が解決済みとみなせる
            init();
        }
        loadSomeTexture();
